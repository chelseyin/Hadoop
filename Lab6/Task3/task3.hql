...
CREATE TABLE IF NOT EXISTS RoseStaticEmployees(firstName string,LastName string,field string,dept string, eid int) PARTITIONED BY (dept String) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS TEXTFILE;
...
CREATE TABLE IF NOT EXISTS RoseDynamicEmployees(firstName string,LastName string,field string,dept string, eid int) PARTITIONED BY (dept String) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS ORC;
SET hive.exec.dynamic.partition.mode=nonstrict;
INSERT INTO TABLE RoseDynamicEmployees Partition(dept) SELECT firstName, LastName, field, eid from RoseStaticEmployees;
CREATE TABLE IF NOT EXISTS RoseStaticEmployeesORC(firstName string,LastName string,field string,dept string, eid int) PARTITIONED BY (dept String) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS ORC;
